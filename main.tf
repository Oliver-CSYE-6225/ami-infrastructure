...
resource "aws_iam_policy" "ghactions_codedeploy" {
    ...
    "Statement" : [
      ...
      },
      {
        ...
        "Resource" : [
          ...
          "arn:aws:codedeploy:${var.aws_region}:${var.aws_account_id}:CodeDeployDefault.HalfAtATime",
          "arn:aws:codedeploy:${var.aws_region}:${var.aws_account_id}:CodeDeployDefault.AllAtOnce"
        ]
      }
    ]
  })
...
resource "aws_iam_role" "gh_actions_role" {
  depends_on          = [aws_iam_policy.gh_upload_to_s3]
  name                = var.gh_actions_role_name
  managed_policy_arns = [aws_iam_policy.gh_upload_to_s3.arn]
  assume_role_policy = jsonencode({
    "Version" : "2012-10-17",
    "Statement" : [
      {
